...
| Frozen objects register (byte count + SHA-256, "no in-place edit; numbered successor") | `registers/json/frozen_objects.json` + `tools/verify_manifests.py` in CI; frozen bodies live under `packages/`/`research/` with `_MANIFEST.jsonl` | CI fails if any frozen byte drifts. Successors are new files; git history preserves predecessors. |
...
| DO_NOT_OPEN vault (superseded mirrors, dead ends, trap copies) | **Not mirrored.** Only the vault's metadata tree is recorded in `drive/vault_tree.txt`; contents are not in this repository | Respects the standing order that models must not consume vault contents for authority. |
...
| Handoff envelope (payload + payload_sha256, noncircular) | `drive/handoffs/*.json` verified by `tools/verify_manifests.py` | Same schema; hash the payload only. |
| Delivery manifest per bundle (one manifest may cover many payloads) | `_MANIFEST.jsonl` per mirrored folder; package `MANIFEST.sha256` files where the source shipped one | Verified in CI. |
