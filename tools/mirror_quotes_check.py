...
The corpus is eligible retained text, not every stored body. Before body reads,
the scan excludes vault, quarantine, legacy and personal lanes, symlinks and
paths outside its root. Manifest-declared Drive IDs and payload digests in
quarantine/EXCLUSIONS.json are excluded too. Exclusion metadata is read as data.
This is an operational quotation check, not scientific review or authority.
Excluded READMEs are outside this check, rather than implicitly approved.

The eligible corpus consists of:

* eligible non-README files under the scanned roots -- the stored Drive payloads and
...
_BLOCKED_PARTS = ('99_DO_NOT_OPEN', '02_LEGACY_Q0_ARCHIVE',
                  '03_PERSONAL_AND_EARLIER_RESEARCH', '90_QUARANTINE_AND_TRIAGE')


def _blocked_path(path):
    parts = str(path).replace('\\', '/').upper().split('/')
    return any(part in ('LEGACY', 'QUARANTINE', 'VAULT', '.GIT', '__PYCACHE__')
               or part.startswith(_BLOCKED_PARTS) for part in parts)


def _safe_path(root, path, *, exclusion_metadata=False):
    """Lexical containment and every in-root symlink checked before reads."""
    root, path = os.path.abspath(root), os.path.abspath(path)
    rel = os.path.relpath(path, root)
    if rel == '..' or rel.startswith('..'+os.sep) or os.path.islink(root):
        return False
    metadata = exclusion_metadata and rel == os.path.join('quarantine', 'EXCLUSIONS.json')
    if not metadata and _blocked_path(rel):
        return False
    cursor = root
...
def _excluded_identities(root):
    path = os.path.join(root, 'quarantine', 'EXCLUSIONS.json')
    if not os.path.lexists(path):
        return set(), set()
    if not _safe_path(root, path, exclusion_metadata=True):
        raise ValueError('exclusion metadata is a symlink or outside the root')
    with open(path, encoding='utf-8') as handle:
        rows = json.load(handle)['exclusions']
    if not isinstance(rows, list) or any(not isinstance(row, dict) for row in rows):
        raise ValueError('invalid exclusion metadata rows')
    ids = {row['carrier_id'] for row in rows
           if row.get('kind') == 'drive_object' and row.get('carrier_id')}
    hashes = {row['payload_sha256'].lower() for row in rows if row.get('payload_sha256')}
    for row in rows:
        if row.get('kind') == 'drive_object':
            hashes.update(re.findall(r'\b[0-9a-f]{64}\b', str(row.get('identity', '')).lower()))
    return ids, hashes


def eligible_files(root, scan_rels):
    """Yield only eligible files; read manifests/holds before any body.

    A matching exclusion removes a source from quotation corroboration without
    reading that source to discover its digest. Hash validation of the declared
    identities remains the manifest checker's separate job.
    """
    ids, hashes = _excluded_identities(root)
    found, excluded = set(), set()
    for scan_rel in scan_rels:
        base = os.path.join(root, scan_rel)
        if not _safe_path(root, base):
            continue
        for dirpath, dirs, files in os.walk(base, followlinks=False):
            dirs[:] = [name for name in dirs if _safe_path(root, os.path.join(dirpath, name))]
            for name in ('_MANIFEST.jsonl', 'MANIFEST.jsonl'):
                manifest = os.path.join(dirpath, name)
                if name not in files:
                    continue
                if not _safe_path(root, manifest):
                    raise ValueError('manifest metadata is a symlink or outside the root')
                with open(manifest, encoding='utf-8') as handle:
                    for line in handle:
                        ...
                        held = (row.get('id') in ids or str(row.get('sha256', '')).lower() in hashes
                                or _blocked_path(row.get('drive_path') or '')
                                or 'HELD_' in str(row.get('status', '')).upper()
                                or 'HELD_' in str(row.get('context', '')).upper())
                        dest = row.get('dest')
                        if held and isinstance(dest, str) and dest:
                            excluded.add(os.path.abspath(os.path.join(dirpath, dest)))
                            excluded.add(os.path.abspath(os.path.join(dirpath, os.path.basename(dest))))
            for name in sorted(files):
                path = os.path.abspath(os.path.join(dirpath, name))
                if path not in excluded and _safe_path(root, path) and os.path.isfile(path):
                    found.add(path)
...
def build_corpus(root, scan_rels, register_rel, inventory_rel, governance_rels):
    ...
    """
    corpus, manifests = [], []
    for path in eligible_files(root, scan_rels):
    ...
    if _safe_path(root, json_dir) and os.path.isdir(json_dir):
        for name in sorted(os.listdir(json_dir)):
            if not name.endswith(".json") or not _safe_path(root, os.path.join(json_dir, name)):
                continue
            try:
    ...
    if _safe_path(root, inventory) and os.path.isfile(inventory):
    ...
    for governance_rel in governance_rels:
        path = os.path.join(root, governance_rel)
        text = readable(path) if _safe_path(root, path) else None
        if text is not None:
...
def readmes(root, scan_rels):
    return [path for path in eligible_files(root, scan_rels) if os.path.basename(path) == 'README.md']
...
def main(argv=None):
    ...
    print(f"mirror_quotes_check: scope=eligible_retained_text readmes={readme_count} fragments={checked} "
