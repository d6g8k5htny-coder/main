...
The fixed-η pair bound is the outer edge for a separate collision question. The [shrinking witness-pair note](rn_d5_shrinking_witness_20260926.md#3-expected-count-bound) keeps both points in that same fixed remote region and lets their mutual separation tend to zero. A divided-difference frame gives an expected ordered same-index count `O(k r^3)`, uniform in the shrinking separation. The fixed-η probability `O(r^6)` stays attached to separations bounded below independently of `r`. The note leaves pin neighborhoods, the intermediate annulus, and elder pairing untouched.
...
| RN fixed-remote successor | Fixed-ρ / fixed-η count review, recorded on the closed issue | [#76](https://github.com/d6g8k5htny-coder/main/issues/76) |
| RN shrinking witness pairs | Divided-difference `m=2` bound at fixed distance from the pins; sharp kernel still open | [Note](rn_d5_shrinking_witness_20260926.md) |
