...
The automated quotation check covers eligible retained text only. Vault,
quarantine, legacy and personal lanes, manifest-declared excluded Drive IDs or
payload digests, and symlinks are excluded before body reads. An excluded README
is outside this check, not implicitly approved. This is operational byte
quotation fidelity, not scientific authority or review. Manifests and inventory
titles remain metadata; their counts do not require opening excluded bodies.
...
piecewise — from each eligible README under the scanned roots, and requires each to appear in eligible stored
...
The checked corpus is eligible retained text:

* eligible non-README files under the scanned roots, meaning stored payloads and the manifests
