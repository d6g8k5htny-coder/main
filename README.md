...
The Canon names a four-object core. This checkout contains exact `Q0_MASTER.md`, `q0_machine.json`, and `q0_verify.py`. The exact `Q0_LEDGER.md` body is **external to this checkout but present on Drive** as file `17gpxTn4MPsrWLf4yhDJsuBE4v3UqQXa3`, 253,067 bytes, SHA-256 `d1fea170ee04c93b075786ed636c3d9088fd80f2c0def51a6a2dbc0812e3e80a`, verified by raw download on 2026-09-25. The local `Q0_LEDGER — NAVIGATION COMPANION ...export.txt` remains a different object and is not relabeled. See `claims/q0_core_availability.json`. Git-only automation must still fail closed if it requires the full ledger body.
