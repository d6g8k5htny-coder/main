...
# ---------------------------------------------------------------------------
# W4. Rigidity-zone integrals (disk d <= 1.5): certified upper bound AND truth
# ---------------------------------------------------------------------------
def make_grid(h, R):
    N = int(math.floor(float(R)/float(h))) + 1
    gxs = [h*i for i in range(-N, N+1)]
    tab = {}
    for si, P in enumerate(PSTATS):
        tab[si] = ([[kL(n, gx - P[0]) for gx in gxs] for n in range(4)],
                   [[kL(n, gy - P[1]) for gy in gxs] for n in range(4)])
    pts = [(ix, iy) for iy, gy in enumerate(gxs) for ix, gx in enumerate(gxs)
           if mp.sqrt(gx*gx + gy*gy) <= R + h/2]
    return gxs, tab, pts

def grid_eval(tab, gxs, ix, iy):
    KZP = mp.matrix(6, 9)
    for i,a in enumerate(DER6):
        for si in range(3):
            Tx, Ty = tab[si]
    ...
    W = KZP * st_s['Spp9i']
    return W*mp.matrix(st_s['vals9']), KZZ - W*KZP.T

R15 = mpf('1.5')
Ics = {}
print("W4 computing certified upper-bound integrals (LB-1 CS form) at mp meshes 0.1 / 0.05 / 0.025 ...")
for h in (mpf('0.1'), mpf('0.05')):
    gxs, tab, pts = make_grid(h, R15)
    tot = mpf(0)
    for ix, iy in pts:
        fa = analysis_from(*grid_eval(tab, gxs, ix, iy))
        tot += fa['wp_exact']
    Ics[float(h)] = tot*h*h
    print("   I_cs(mesh %s) = %s  (%d cells)" % (h, mp.nstr(Ics[float(h)], 8), len(pts)))
# mesh 0.025: single cached pass feeding W4 (CS integral), W6 (kill/rim), W7 (mass geometry)
gxs25, tab25, pts25 = make_grid(mpf('0.025'), R15)
FA25 = []
tot = mpf(0)
for ix, iy in pts25:
    FA25.append(analysis_from(*grid_eval(tab25, gxs25, ix, iy)))
    tot += FA25[-1]['wp_exact']
Ics[0.025] = tot*mpf('0.025')**2
print("   I_cs(mesh 0.025) = %s  (%d cells)" % (mp.nstr(Ics[0.025], 8), len(pts25)))
ck(abs(Ics[0.05]/Ics[0.025] - 1) < mpf('0.02'), "W4a CS-bound integral mesh-stability < 2% (0.05 vs 0.025)")
ck(mpf('1.2e-5') < Ics[0.025] < mpf('1.4e-5'),
   "W4b certified upper-bound zone integral = %s (LB-1's ~1.3e-5 CONFIRMED as a valid upper bound)" % mp.nstr(Ics[0.025],6))

Itr = {}
RT5 = None
print("W4 computing two-sided TRUE integrals at meshes 0.1 / 0.05 (u-slice engine, gh16, n_u=41) ...")
for h in (mpf('0.1'), mpf('0.05')):
    gxs, tab, pts = make_grid(h, R15)
    tot = 0.0
    rt_cells = []
    for ix, iy in pts:
        fa = analysis_from(*grid_eval(tab, gxs, ix, iy))
        rt = u_slice(*cond_params(fa), ngh=16, n_u=41)
        tot += rt
        rt_cells.append(rt)
    Itr[float(h)] = tot*float(h)**2
    if float(h) == 0.05: RT5 = (gxs, pts, rt_cells)
    print("   I_true(mesh %s) = %.6e  (%d cells)" % (h, Itr[float(h)], len(pts)))
ck(abs(Itr[0.1]/Itr[0.05] - 1) < 0.05, "W4c true integral mesh-stability < 5% (0.1 vs 0.05)")

# fine strip (dominant mass): x in [-0.6,0.6], y in [0.25,1.3], mesh 0.0125
h12 = mpf('0.0125')
xs12 = [h12*i for i in range(-48, 49)]
ys12 = [h12*j for j in range(20, 105)]
tab12 = {si: ([[kL(n, gx - PSTATS[si][0]) for gx in xs12] for n in range(4)],
              [[kL(n, gy - PSTATS[si][1]) for gy in ys12] for n in range(4)]) for si in range(3)}
Istrip = 0.0; stripmax = 0.0; striparg = None
for iy, gy in enumerate(ys12):
    for ix, gx in enumerate(xs12):
        if mp.sqrt(gx*gx+gy*gy) > R15: continue
        KZP = mp.matrix(6, 9)
        for i,a in enumerate(DER6):
            for si in range(3):
                Tx, Ty = tab12[si]
                for cidx, c in enumerate(J01):
                    KZP[i, si*3+cidx] = ((-1)**(c[0]+c[1])) * Tx[a[0]+c[0]][ix] * Ty[a[1]+c[1]][iy]
        W = KZP * st_s['Spp9i']
        fa = analysis_from(W*mp.matrix(st_s['vals9']), KZZ - W*KZP.T)
        rt = u_slice(*cond_params(fa), ngh=16, n_u=41)
        Istrip += rt
        if rt > stripmax: stripmax, striparg = rt, (float(gx), float(gy))
Istrip *= float(h12)**2
print("   I_strip(mesh 0.0125, x in [-0.6,0.6], y in [0.25,1.3]) = %.6e" % Istrip)
print("   TRUE hot spot: rho_true = %.6e/unit-area at (%.4f, %.4f)" % (stripmax, striparg[0], striparg[1]))
# out-of-strip remainder from the cached 0.05 grid pass
gxs, pts, rt_cells = RT5
Iout = 0.0
for (ix, iy), rt in zip(pts, rt_cells):
    gx, gy = gxs[ix], gxs[iy]
    if abs(float(gx)) <= 0.6 and 0.25 <= float(gy) <= 1.3: continue
    Iout += rt
Iout *= 0.0025
I_true = Istrip + Iout
print("   out-of-strip remainder = %.6e ; I_true(zone) = I_strip + remainder = %.6e" % (Iout, I_true))
ck(4.4e-6 < I_true < 5.2e-6, "W4d TRUE rigidity-zone integral = %.4e (mesh-stable, derived-on-grid grade)" % I_true)

# ---------------------------------------------------------------------------
# W5. Ruling arithmetic
# ---------------------------------------------------------------------------
print("W5 RULING:")
print("   C030/C031 printed rigidity-zone figure : ~3e-15 (3.6e-15 in C031 line 29)")
print("   certified TRUE zone integral           : %.4e  (derived-on-grid)" % I_true)
print("   certified UPPER BOUND zone integral    : %s  (LB-1 CS form)" % mp.nstr(Ics[0.025],6))
ck(I_true/mpf('3.6e-15') > 1e8,
   "W5a C030's rigidity-zone figure 3.6e-15 is REFUTED as the zone value: truth is ~%.1e times larger (derived-on-grid grade)" % (I_true/3.6e-15))
ck(I_true > mpf('3.32e-6'),
   "W5b the printed WP channel total 0.213*r^3 = 3.32e-6 is FALSIFIED as an upper bound at r = 0.025: the rigidity-zone truth %.4e alone exceeds it (factor %.2f)" % (I_true, I_true/mpf('3.32e-6')))
ck(I_true < Ics[0.025], "W5c consistency: truth %.4e < certified upper bound %s (LB-1's recheck bound VALID, loose by factor %.1f)" % (I_true, mp.nstr(Ics[0.025],6), float(Ics[0.025])/I_true))
for nm, pt, kant in (("P*", (mpf('1.30561'), mpf('0.68576')), 1.93119035755789484),
                     ("Q*", (mpf('-1.10400'), mpf('0.87969')), 1.66659821936183025)):
    mm, cc = cond_law(st_s, pt)
    print("   m(%s) = %s  (Task 4c Kantorovich: %.17f)" % (nm, mp.nstr(mm[0], 15), kant))
    ck(mm[0] > BB, "W5d m(%s) > b: C030's zone-wide value-kill premise is CERTIFIED FALSE at %s" % (nm, nm))
    ck(abs(float(mm[0]) - kant) < 1e-9, "W5e cross-agent agreement with KIMI-DER-027c Kantorovich value at %s (< 1e-9)" % nm)

# ---------------------------------------------------------------------------
# W6. Code-path origin of the bad print
# ---------------------------------------------------------------------------
print("W6 code-path origin: C030's figure derives from its consequence (i): the zone-wide")
print("   kill e^{-(b-m)^2/2v} 'throughout d <~ 1.5'. Scanning the cached zone (mesh 0.025):")
killmin = mpf(1); kill_arg = None; kill_close = None; kill_close_d = mpf(1)
rim_I = mpf(0); rim_n = 0; tot_I = mpf(0)
for (ix, iy), fa in zip(pts25, FA25):
    gx, gy = gxs25[ix], gxs25[iy]
    tot_I += fa['wp_exact']
    s = mp.sqrt(fa['v_t'])
    ustar = min(max(fa['mu_t'], BB-ell), BB)
    if abs(ustar - fa['mu_t']) <= 2*s:
        rim_I += fa['wp_exact']; rim_n += 1
    kl = mp.e**(-(BB-fa['m'])**2/(2*fa['v'])) if fa['m'] < BB else mpf(1)
    if kl < killmin: killmin, kill_arg = kl, (gx, gy, fa['m'], fa['v'])
    d = abs(kl - mpf('3.6e-15'))
    if d < kill_close_d: kill_close_d, kill_close = d, (kl, gx, gy, fa['m'], fa['v'])
rim_I *= mpf('0.025')**2; tot_I *= mpf('0.025')**2
print("   pointwise kill min over zone: %s at (%.3f, %.3f) (m = %.5f, v = %.2e)" % (mp.nstr(killmin,3), float(kill_arg[0]), float(kill_arg[1]), float(kill_arg[2]), float(kill_arg[3])))
kl0, x0, y0, m0, v0 = kill_close
print("   zone cell with kill closest to C031's 3.6e-15: kill = %s at (%.3f, %.3f), m = %.5f, v = %.2e, (b-m)^2/2v = %.2f" % (
    mp.nstr(kl0,3), float(x0), float(y0), float(m0), float(v0), float((BB-m0)**2/(2*v0))))
ck(1e-15 < float(kl0) < 1e-14,
   "W6a C030's 3.6e-15 figure class REPRODUCED as a pointwise interior kill value -- the bad print's origin: a pointwise kill evaluated deep inside the horizon (small v, m ~ 1.117) was extended zone-wide, while the rim band was invisible to the premise")
print("   rim band {mu_t within 2 sd of window}: %d cells (area %.4f) carrying %s of the upper-bound integral (%.1f%%)" % (
    rim_n, rim_n*0.025**2, mp.nstr(rim_I,6), float(100*rim_I/tot_I)))
ck(float(rim_I/tot_I) > 0.90, "W6b rim band carries > 90% of the upper-bound zone integral")

# ---------------------------------------------------------------------------
# W7. Propagation old -> new; LB-1 sup-over-zone survival
# ---------------------------------------------------------------------------
r3 = r**3
old_zone = mpf('3.6e-15'); old_tot = mpf('0.213')*r3           # = 3.328125e-6
new_zone_true = I_true; new_zone_ub = Ics[0.025]
trans = mpf('1.95e-6'); far = mpf('1.37e-6')                   # C031 prints, this script's scope excludes re-deriving them
new_tot_true = new_zone_true + trans + far
new_tot_ub = new_zone_ub + trans + far
print("W7 PROPAGATION (r = 0.025 rung of record; r^3 = %s):" % mp.nstr(r3,6))
print("   [rigidity zone]  old ~3e-15 (= %s*r^3)  ->  new truth %.4e (= %s*r^3); certified ub %s (= %s*r^3)" % (
    mp.nstr(old_zone/r3,3), new_zone_true, mp.nstr(new_zone_true/r3,4), mp.nstr(new_zone_ub,6), mp.nstr(new_zone_ub/r3,4)))
print("   [WP channel]     old total 0.213*r^3 = %s  ->  new truth %.4e (= %s*r^3); certified ub %.4e (= %s*r^3)" % (
    mp.nstr(old_tot,6), new_tot_true, mp.nstr(new_tot_true/r3,4), new_tot_ub, mp.nstr(new_tot_ub/r3,4)))
print("   [AO_lb ledger]   old 0.9666 - (0.1759 + 0.213 + eps)*r^3  ->  new 0.9666 - (0.1759 + %s .. %s + eps)*r^3" % (
    mp.nstr(new_tot_true/r3,4), mp.nstr(new_tot_ub/r3,4)))
print("   [c limit]        UNCHANGED: every correction is O(r^3) and vanishes as r -> 0; c -> 0.9666*0.946 = 0.9144 stands")
print("   [G-F2a rung ratios 7.76/8.19] consumed the TRANSITION piece's ell-scaling, not the rigidity figure:")
print("      unaffected. The rigidity piece's Pw ~ (ell/sf)*phi scaling is ALSO ell-linear at fixed geometry, so the")
print("      ratio architecture survives; the CONSTANT changes as displayed. Recomputation at other rungs is Task 5's.")
print("   [AUD-023 E/tol rung chain 1.7e-23 -> 2.5e-506] belongs to the above-b saddle channel (B_{3r} zone); it never")
print("      consumed the WP value. Untouched.")

# LB-1 survival: geometric + functional separation
mind = mpf(10)
for (ix, iy), fa in zip(pts25, FA25):
    gx, gy = gxs25[ix], gxs25[iy]
    if fa['wp_exact'] > mpf('1e-8'):
        d = mp.sqrt(gx*gx+gy*gy)
        if d < mind: mind = d
ck(mind > 3*r, "W7a min zone-d of significant (wp > 1e-8) mass = %s > 3r = %s" % (mp.nstr(mind,4), mp.nstr(3*r,4)))
dA = mp.sqrt((WA[0]-StM[0])**2 + (WA[1]-StM[1])**2)
dH = mp.sqrt((mpf('0')-StM[0])**2 + (mpf('0.5875')-StM[1])**2)
print("   witness A at d = %s from M; true hot spot at d = %s from M; LB-1's zone is B_{3r}, r = 0.025: radius %s" % (
    mp.nstr(dA,4), mp.nstr(dH,4), mp.nstr(3*r,4)))
ck(dA > 3*r and dH > 3*r, "W7b the witness point and the true hot spot lie OUTSIDE LB-1's zone B_{3r}")
print("W7c LB-1's sup-over-zone conclusion SURVIVES: (i) its certificate [B] is a self-contained")
print("    exact-kernel computation over B_{3r}(M) of the ABOVE-B CRITICAL-POINT intensity bound")
print("    (E/tol = 1.7e-23 ... 2.5e-506 at six rungs, transcripts byte-identical both modes);")
print("    (ii) the WP correction concerns WINDOW-SADDLES over d <= 1.5 -- a different event")
print("    functional and a different zone; (iii) the mass corrected here sits at d >= %s, more" % mp.nstr(mind,3))
print("    than 5x outside B_{3r}. Nothing in this certificate touches LB-1's zone or functional.")

print("")
print("CERTIFICATE: PASS  (WO-063 Task 1 WP resolution; all checks fail-closed)")
print("Headline: rigidity-zone window-saddle truth %.4e (derived-on-grid), certified ub %s;" % (I_true, mp.nstr(Ics[0.025],6)))
print("C030's 3e-15 and the printed 0.213*r^3 channel total refuted; LB-1 unaffected.")
