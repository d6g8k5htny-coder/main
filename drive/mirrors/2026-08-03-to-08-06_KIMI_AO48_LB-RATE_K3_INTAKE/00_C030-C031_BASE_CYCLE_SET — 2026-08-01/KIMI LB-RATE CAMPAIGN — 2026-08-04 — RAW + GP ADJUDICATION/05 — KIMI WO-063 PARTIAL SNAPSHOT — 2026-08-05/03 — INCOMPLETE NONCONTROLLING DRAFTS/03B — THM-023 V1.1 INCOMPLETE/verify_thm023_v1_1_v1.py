#!/usr/bin/env python3
"""
verify_thm023_v1_1_v1.py -- AO48-WO-063 Task 5 (THM-023 v1.1 ASSEMBLY) certificate.
...
def rung_wp(rval, h='0.05'):
    ...
    for iy, gy in enumerate(gxs_):
        for ix, gx in enumerate(gxs_):
            ...
            Ics_ += fa['wp_exact']; It += u_slice(*cond_params(fa), ngh=16, n_u=41)
    return It*float(hh)**2, Ics_*hh**2

print("T5-1 WP rung table + inter-rung midpoints (mesh 0.05) ...")
RUNGS = ['0.05','0.045','0.04','0.035','0.03','0.025','0.0225','0.02','0.0175','0.0125','0.011','0.01','0.009','0.008','0.007','0.006','0.0045','0.0035','0.003','0.0025']
tbl = {}
for rv in RUNGS:
    It, Ics_ = rung_wp(rv)
    tbl[rv] = (It, Ics_)
    print("   r=%-6s truth=%.5e  ub=%s  coeff_cs=%s" % (rv, It, mp.nstr(Ics_,5), mp.nstr(Ics_/mpf(rv)**3,5)), flush=True)

print("T5-2 envelope gates: WP(r) <= E(r) = 5.5e-3 * r^1.6 at every computed r")
ENV = mpf('5.5e-3')
worst = None
for rv,(It, Ics_) in tbl.items():
    r_ = mpf(rv); env = ENV*r_**mpf('1.6')
    ck(Ics_ < env, "T5-2 ub(%s) = %s < envelope %s (margin %s)" % (rv, mp.nstr(Ics_,4), mp.nstr(env,4), mp.nstr(env/Ics_,4)))
    ck(It < float(env), "T5-2b truth(%s) below envelope" % rv)
    worst = env/Ics_ if worst is None else min(worst, env/Ics_)
print("   worst envelope margin over the table:", mp.nstr(worst,4))
ck(worst > mpf('1.05'), "T5-2c envelope margin >= 1.05 at every computed r")

print("T5-3 exponent-law stability (ub * r^1.4 across the small-r decade):")
vals = []
for rv in ('0.0125','0.01','0.008','0.006','0.005','0.0035','0.0025'):
    v = tbl[rv][1]*mpf(rv)**mpf('1.4')
    vals.append(v)
    print("   r=%-7s ub*r^1.4 = %s" % (rv, mp.nstr(v,4)))
ck(all(mpf('4.5e-3') < v < mpf('5.2e-3') for v in vals), "T5-3 exponent law ub*r^1.4 in (4.5e-3, 5.2e-3) over the decade")

print("T5-4 assembly ledger (exact arithmetic):")
r0 = mpf('0.05'); r03 = r0**3
R2c = mpf('0.1759'); WPub05 = tbl['0.05'][1]
AO0 = mpf('0.9666'); cL = mpf('0.946')
AOlb = AO0 - R2c*r03 - WPub05 - mpf('1.7e-23')
print("   AO_lb(0.05) = 0.9666 - 0.1759*r0^3 - WP_ub(0.05) - 1.7e-23 =", mp.nstr(AOlb, 12))
ck(AOlb > mpf('0.9665'), "T5-4a AO_lb(0.05) > 0.9665")
c_r0 = AOlb*cL
print("   c(0.05) = AO_lb * c_L (0.946 measured) =", mp.nstr(c_r0, 12))
ck(c_r0 > mpf('0.914'), "T5-4b c(0.05) > 0.914 at c_L = 0.946")
c_lim = AO0*cL
print("   limit c -> 0.9666 * 0.946 =", mp.nstr(c_lim, 12))
ck(abs(c_lim - mpf('0.9144036')) < mpf('1e-7'), "T5-4c limit constant 0.9144036")
print("   honest-constant move: c(0.05) = %s vs limit %s (4th-5th decimal, displayed)" % (mp.nstr(c_r0,10), mp.nstr(c_lim,10)))
print("")
print("CERTIFICATE: PASS  (WO-063 Task 5 assembly gates; WP envelope + ledger)")
