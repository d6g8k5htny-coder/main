...
For a 3-uniform minimal-witness hypergraph with maximum pair-codegree
(D`\ge1`{=tex}), \[
`\mu`{=tex}\_{`\mathbf `{=tex}p}(`\mathcal `{=tex}D)`\ge3`{=tex}/4
`\Rightarrow`{=tex} `\boxed{
\mathcal D^{(\lceil11\sqrt D\rceil)}
\text{ is product-measure small}.}`{=tex} \] The proof permits unbounded
vertex degree and arbitrary crossing geometry. It extracts vertices
whose extension graphs have mass (\>12D), then applies PR-TAL-016 to the
residual dependency load (c`\le`{=tex}39D). The certified total cost is
(\<1409/2904\<1/2).

The remaining rank-three barrier is unbounded pair-codegree. Pair-core
extraction is the next target.
