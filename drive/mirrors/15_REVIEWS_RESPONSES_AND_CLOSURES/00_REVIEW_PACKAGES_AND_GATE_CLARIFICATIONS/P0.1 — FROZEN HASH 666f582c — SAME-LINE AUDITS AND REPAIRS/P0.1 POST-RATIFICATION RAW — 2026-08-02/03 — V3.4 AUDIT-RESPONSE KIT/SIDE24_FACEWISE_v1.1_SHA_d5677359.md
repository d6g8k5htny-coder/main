...
Write (a=\eta/2).  The row (6.3) gives (2A); after (A=0), the
witness value-gradient matrix has determinant (\sigma^6).  Thus this
interpolation remains uniform through (\eta=0) and (c=0).  The confluent
third-order endpoint row is

[
T_4(p):=\frac3{a^2}\left[
\frac{p_X(a)+p_X(-a)}2-\frac{p(a)-p(-a)}{2a}\right],
\qquad \lim_{a\to0}T_4(p)=p_{XXX}(0).                     \tag{6.4a}
]

Use (T_4), rather than the literal (X)-gradient average, in the endpoint
value/(X)-gradient average/divided-difference rows through order three;
retain the (Y)-gradient rows and use
([p_{XY}(a)-p_{XY}(-a)]/(2a)) as the seventh confluent row.  With the
three witness rows, this normalized ten-by-ten determinant is exactly
(-24\sigma^6), independent of (\eta,c).  By contrast, the literal
average/divided-difference reading without (T_4) has determinant
(2\eta^2\sigma^6).  These statements concern different row
normalizations: the original node value/gradient matrix, with the mixed
row already normalized as in (6.3), carries the full (\eta^5)
confluent-row Jacobian; the literal partial normalization removes three
powers of (\eta) and leaves the displayed (2\eta^2), while the
(T_4)-normalization removes the remaining two.  None is a covariance
loss.
...
On the singular mixed/collinear charts the two regimes are

[
\mathcal K_r(y,u)
\le
\begin{cases}
Cs^{-7}\mathcal D(r,s)s^{-6}\Theta^{-N_0}e^{-c_0/\Theta^2}
\le Cs^{-7}\mathcal D(r,s)\Theta^{-N}e^{-c/\Theta^2},
&\varrho\lesssim s\quad(\Theta\asymp s;\ \hbox{near axis}),\\
Cs^{-7}\mathcal D(r,s)\Theta^{-N}e^{-c/\Theta^2},
&\varrho\gtrsim s\quad(\hbox{angular-dominant overlap; use (6.2)}),
\end{cases}
\qquad \Theta^2=s^2+\varrho^2.                          \tag{6.7}
]

On the collar axial charts separated from the endpoint-collision divisor,
after the one legitimate value integration, the corresponding two regimes
are
...
\le
\begin{cases}
Cr^{-4}\mathcal D(r,s)r^{-3}\Theta^{-N_0}e^{-c_0/\Theta^2}
\le Cr^{-4}\mathcal D(r,s)\Theta^{-N}e^{-c/\Theta^2},
&\Theta\lesssim s,\ s\asymp r\quad(\hbox{near-axis/core}),\\
Cr^{-4}\mathcal D(r,s)\Theta^{-N}e^{-c/\Theta^2},
&\Theta\gtrsim s\quad(\hbox{angular-dominant overlap; use (6.2)}).
\end{cases}                                               \tag{6.8}
]
